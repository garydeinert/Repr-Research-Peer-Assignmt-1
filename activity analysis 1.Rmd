...
hist(dayTotal$x, main = "Histogram of Daily Walking Totals", xlab = "Daily Steps")
...
hist(newTotal$x, main = "Updated Histogram of Daily Walking Totals", xlab = "Daily Steps")


```
   
***  __The average number of steps per day is `r newMean` and the median number of steps per day is `r newMedian`.  Compare to the prior average steps per day at `r dayMean` and previous median of `r dayMedian`.   We conclude that the impact of imputing values by the interval mean does not significantly change the results.__   
